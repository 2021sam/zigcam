...
function startRecording() {
    exec(`your_recording_command --output ${videoFilePath}`, (error, stdout, stderr) => {
        if (error) {
...
function stopRecording() {
    exec('your_stop_recording_command', (error, stdout, stderr) => {
        if (error) {
...
setTimeout(stopRecording, 5000); // Stop after 60 seconds
